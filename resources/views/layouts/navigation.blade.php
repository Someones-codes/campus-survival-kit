    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center">
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center gap-2">
                ...
                <span class="font-sans font-semibold text-ink hidden sm:inline">Survival Kit</span>
            </a>
            ...
            <div class="hidden sm:flex items-center gap-2">
                ...
                <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                    <button @click="open = !open" type="button"
                        class="font-mono text-xs uppercase tracking-widest px-3 py-2 rounded-sm text-ink-light hover:bg-ink/5 flex items-center gap-1 {{ request()->routeIs('budgets.*') || request()->routeIs('transactions.index') || request()->routeIs('categories.*') || request()->routeIs('reports.*') ? 'bg-ink/5' : '' }}">
                        More
                    ...
                    <div x-show="open" x-cloak
                        ...
                        <a href="{{ route('reports.index') }}"
                            class="block px-4 py-2 text-sm text-ink hover:bg-paper-dark/40 {{ request()->routeIs('reports.*') ? 'font-semibold' : '' }}">
                            Recon Reports
                        </a>
                    </div>
                </div>
        ...
        <div x-show="mobileOpen" x-cloak class="sm:hidden pb-4 space-y-1">
            ...
            <a href="{{ route('reports.index') }}" class="block px-3 py-2 rounded-sm text-sm {{ request()->routeIs('reports.*') ? 'bg-ration-green text-paper' : 'text-ink hover:bg-ink/5' }}">Recon Reports</a>
